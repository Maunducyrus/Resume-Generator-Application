...
export const cvAPI = {
  ...
  shareCV: async (
    id: string,
    isPublic: boolean,
  ): Promise<{ shareUrl?: string; isPublic: boolean }> => {
    try {
      const response = await api.put(`/cv/${id}/share`, { isPublic });
      return response.data.data;
    } catch (error: any) {
      throw new Error(
        error.response?.data?.message || "Failed to update sharing settings",
      );
    }
  },
  ...
  getSharedCV: async (shareUrl: string): Promise<CV> => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to get shared CV",
      );
    }
  },
};
...
export const aiAPI = {
  ...
  optimizeExperience: async (data: {
    ...
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to optimize experience",
      );
    }
  },
  ...
  optimizeForJob: async (data: {
    ...
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to optimize for job",
      );
    }
  },

  generateSkillSuggestions: async (data: {
    profession: string;
    experience?: any[];
  }): Promise<string[]> => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to generate skill suggestions",
      );
    }
  },
};
