...
const TemplateGallery: React.FC<TemplateGalleryProps> = ({ onViewChange }) => {
  ...
  const filteredTemplates = templates.filter((template) => {
    ...
    const matchesSearch =
      ...
      template.professions.some((prof) =>
        ...
        <div className="flex items-center justify-between">
          <div className="text-xs text-gray-500">
            ...
            {template.professions.length > 2 &&
              ` +${template.professions.length - 2} more`}
          </div>
          <button
        ...
        <div className="text-center mb-12">
          ...
          >
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Select from our collection of professional, ATS-optimized
                ...
                <div key={category} className="text-center">
                  ...
                  <p className="text-sm text-gray-600">
                    {category === "Professional" &&
                      "Clean, corporate designs for traditional industries"}
                    {category === "Creative" &&
                      "Bold, artistic layouts for creative professionals"}
                    {category === "Tech" &&
                      "Modern designs optimized for tech roles"}
                    {category === "Minimal" &&
                      "Simple, elegant templates with focus on content"}
                    {category === "Academic" &&
                      "Traditional formats for academic positions"}
                    {category === "Healthcare" &&
                      "Professional designs for medical professionals"}
                    {category === "Sales" &&
                      "Results-focused templates for sales roles"}
                    {category === "Marketing" &&
                      "Creative templates for marketing professionals"}
                  </p>
                </div>
              );
            })}
